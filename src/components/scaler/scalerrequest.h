...
public:
    ...
    ScalerRequest(std::shared_ptr<Image> _image, QSize _size, QString _path, ScalingFilter _filter) : image(std::move(std::move(_image))), size(_size), path(std::move(std::move(_path))), filter(_filter) {}
